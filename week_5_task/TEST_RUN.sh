...
cd ./llvm-basic-pass/build
make
cd ../..

./build/bin/clang -S -o0 -emit-llvm llvm-basic-pass/tests/input.c -o input.ll
./build/bin/opt -load-pass-plugin ./llvm-basic-pass/build/libMyPass.so -passes=MyPass -disable-output input.ll
